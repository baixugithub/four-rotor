...
static void i2cUnstick(void)
{
    ...
    for (i = 0; i < 8; i++) {
        ...
        while (!GPIO_ReadInputDataBit(gpio, scl))
            delay(10);

        // Pull low
        GPIO_ResetBits(gpio, scl); // Set bus low
        delay(10);
        // Release high again
        GPIO_SetBits(gpio, scl); // Set bus high
        delay(10);
    }
    ...
    // SCL  PB10
    // SDA  PB11
    GPIO_ResetBits(gpio, sda); // Set bus data low
    delay(10);
    GPIO_ResetBits(gpio, scl); // Set bus scl low
    delay(10);
    GPIO_SetBits(gpio, scl); // Set bus scl high
    delay(10);
    GPIO_SetBits(gpio, sda); // Set bus sda high
