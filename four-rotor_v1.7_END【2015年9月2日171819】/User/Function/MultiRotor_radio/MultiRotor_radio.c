...
unsigned char HtoEs_Attitude_Data_Generate(void)
{
	  ...
	  Float2Byte(&AngE.Pitch,HtoEs_OutPut_Buffer,2);  //俯仰
	  Float2Byte(&AngE.Roll ,HtoEs_OutPut_Buffer,6);  //横滚
	  Float2Byte(&AngE.Yaw  ,HtoEs_OutPut_Buffer,10); //航向
...
unsigned char HtoEs_RC_Data_Generate(void)
{
	  ...
	  HtoEs_OutPut_Buffer[0] = 0x15; //帧长度 15字节
		HtoEs_OutPut_Buffer[1] = 0x04; //功能码
...
void mavlink(void)
{
	switch(sw) //循环发送各模块数据
	{
			...
			case 2: F = HtoEs_RC_Data_Generate();       break;  //测试RC通道，返回需发送字节数		 
